    ...
    def __init__(self, graph_file, emb_file1, emb_file2, dimension):
        ...
        with open(graph_file, 'r') as rf:
            for i, line in enumerate(rf):
                if i % 1000000 == 0:
                    logger.info('read edge list line %d', i)
                line = line.strip()
                items = line.split()
                src = np.int64(items[0])
                dst = np.int64(items[1])
                ...
                if src != dst:
                    ...
                    rows.append(dst)
                    cols.append(src)
        self.node_number = maxId + 1
        rows = np.array(rows, dtype=np.int64)
        cols = np.array(cols, dtype=np.int64)
        vals = np.ones((rows.shape[0],))
        self.matrix0 = scipy.sparse.csr_matrix((vals, (rows, cols)),
                                               shape=(self.node_number, self.node_number))
