...
DEFINE_int64(num_node, 4, "Number of node in the graph.");
...
SMatrixXf readGraph(string filename, long num_node){
    ...
    while (1)
    {
        ...
        if (!(fin >> x >> y >> z))
            break;
        lineCnt++; 
        if (lineCnt % 10000000 == 0) cout << "Reading graph line " << lineCnt << ", reading time:" << (time(NULL) - start_time + 0.0) << endl;
        //int a = atoi(x.c_str()), b = atoi(y.c_str());
        long a = strtoll(x.c_str(), NULL, 10), b = strtoll(y.c_str(), NULL, 10);
        maxID = max(a, maxID);
        maxID = max(b, maxID);
        if (a==b) continue;
    ...
    long nnz = tripletList.size();
    cout << "Number of Edges (nnz): " << nnz << endl;
    A.reserve(nnz);
    A.setFromTriplets(tripletList.begin(), tripletList.end());
    //A.resize(nnz, maxID);
    /*
    for (int i = 0; i < nnz; ++i) {
        if (i % 1000000 == 0) cout << "Setting edge " << i << endl;
        T curEdge = tripletList[i];
        int curRow = curEdge.row();
        A.startVec(curRow);
        cout << curRow << " " << curEdge.col() << " " << curEdge.value() << endl;
        A.insertBack(curRow, curEdge.col()) = curEdge.value();
    }
    A.finalize();
    */
    return A;
